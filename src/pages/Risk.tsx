...
const riskEvents = [
  ...
  { id: "RE004", customer: "Neha Gupta", type: "Payment Delay", severity: "High", time: "3 hours ago", status: "Open" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button className="bg-gradient-primary hover:opacity-90" onClick={() => setIsNewRuleOpen(true)}>
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-3">
            {["Price Drop Alert", "LTV Breach Protocol", "Overdue Escalation"].map((template, idx) => (
              <Button key={idx} variant="outline" className="justify-start text-sm">
                {template}
              </Button>
            ))}
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">System Action</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-muted-foreground">Manager Action</th>
                </tr>
              </thead>
              <tbody>
                {riskEvents.map((event) => (
                  <tr key={event.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-sm">
                      <Badge className={`${getSeverityColor(event.severity)} font-medium`}>
                        {event.severity}
                      </Badge>
                    </td>
                    ...
                    <td className="py-3 px-4 text-sm text-muted-foreground">{getSystemAction(event.severity)}</td>
      ...
      <Dialog open={isNewRuleOpen} onOpenChange={setIsNewRuleOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-6">
            ...
            <Button className="w-full bg-gradient-primary hover:opacity-90" onClick={handleSaveRule}>
              Save Risk Rule
            </Button>
          </div>
        </DialogContent>
